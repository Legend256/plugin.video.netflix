...
class AMStreamContinuity(ActionManager):
    ...
    def on_playback_started(self, player_state):
        ...
        if (player_state.get(STREAMS['subtitle']['current']) is None and
            ...
            # KODI BUG! All Kodi Matrix versions <= 19.4 have the playlist bug, fixed in versions >= 19.5
            # These versions set a wrong playlist id and corrupt the response data in JSON-RPC calls.
            self.enabled = False
            LOG.error('You are using a bugged Kodi version, please update to Kodi 19.5 or above: '
            ...
            ui.show_notification(title=common.get_local_string(30105),
                                 msg='To use Netflix add-on features update to Kodi 19.5 or above.')
            return
